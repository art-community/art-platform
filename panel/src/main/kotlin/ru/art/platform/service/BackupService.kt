...
import ru.art.platform.api.mapping.configuration.PreparedConfigurationMapper
...
import ru.art.platform.repository.PreparedConfigurationsRepository.getPreparedConfigurations
import ru.art.platform.repository.ProjectRepository.getProjects
import ru.art.platform.service.ManagementService.getVersion
        ...
        getProjects().forEach { project ->
            try {
                ...
                getPreparedConfigurations().forEach { configuration ->
                    val path = "${path.toAbsolutePath()}/platform-backup/${getVersion()}/${configuration.profile}-${configuration.name}.json"
                    writeJson(fromPreparedConfiguration.map(configuration), get(path))
                }
                add(backupDirectory)
