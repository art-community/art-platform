...
task("buildDockerImage", type = Exec::class) {
    ...
    dependsOn += "build"
    if (os.current().isWindows) {
        commandLine("cmd", "/c", "docker build --rm -t platform/management-panel:$version .")
        return@task
    }
    commandLine("docker", "build", "--rm", "-t", "platform/management-panel:$version", ".")
}
...
dockerProjects.let { projects ->
    projects.split(",").forEach { dockerProject ->
        task("$dockerProject-tag", type = Exec::class) {
            ...
            if (os.current().isWindows) {
                commandLine("cmd", "/c", "docker tag platform/management-panel:$version $dockerRegistryUrl/$dockerProject/management-panel:$version")
                return@task
            }
            commandLine("docker", "tag", "platform/management-panel:$version", "$dockerRegistryUrl/$dockerProject/management-panel:$version")
        }
        ...
        task("$dockerProject-push", type = Exec::class) {
            ...
            if (os.current().isWindows) {
                commandLine("cmd", "/c", "docker push $dockerRegistryUrl/$dockerProject/management-panel:$version")
                return@task
            }
            commandLine("docker", "push", "$dockerRegistryUrl/$dockerProject/management-panel")
        }
    }
}
